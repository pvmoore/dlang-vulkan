module vulkan.imgui.imgui_glfw;
 ...
 *
 * NOTE(1)
 *   GetWindowPos and GetWindowSize return ImVec2 which is not supported by C. We call
 *   helper functions in the cimgui code to set wrappers for these
 *
 *
 *
 */
...
enum __linux__      = false;

static if(_WIN32) {
    // #ifdef _WIN32
    // #undef APIENTRY
    // #ifndef GLFW_EXPOSE_NATIVE_WIN32
    enum GLFW_EXPOSE_NATIVE_WIN32 = true;
    // #endif
}
...
enum GLFW_EXPOSE_NATIVE_COCOA = false;
// #endif
...
// #else
enum EMSCRIPTEN_USE_EMBEDDED_GLFW3 = false;
// #endif
// #endif
...
enum GLFW_HAS_PER_MONITOR_DPI    = (GLFW_VERSION_COMBINED >= 3300); // 3.3+ glfwGetMonitorContentScale
...
// #else
enum GLFW_HAS_VULKAN             = (GLFW_VERSION_COMBINED >= 3200); // 3.2+ glfwCreateWindowSurface
// #endif
enum GLFW_HAS_FOCUS_WINDOW       = (GLFW_VERSION_COMBINED >= 3200); // 3.2+ glfwFocusWindow
enum GLFW_HAS_FOCUS_ON_SHOW      = (GLFW_VERSION_COMBINED >= 3300); // 3.3+ GLFW_FOCUS_ON_SHOW
enum GLFW_HAS_MONITOR_WORK_AREA  = (GLFW_VERSION_COMBINED >= 3300); // 3.3+ glfwGetMonitorWorkarea
enum GLFW_HAS_OSX_WINDOW_POS_FIX = (GLFW_VERSION_COMBINED >= 3301); // 3.3.1+ Fixed: Resizing window repositions it on MacOS #1553
...
enum GLFW_HAS_GETPLATFORM       = (GLFW_VERSION_COMBINED >= 3400); // 3.4+ glfwGetPlatform()
...
void ImGui_ImplGlfw_ContextMap_Remove(GLFWwindow* window) { for(auto i=0; i < g_ContextMap.Size; i++) { ImGui_ImplGlfw_WindowToContext* entry = &g_ContextMap[i]; if (entry.Window == window) { g_ContextMap.erase_unsorted(entry); return; } } }
ImGuiContext* ImGui_ImplGlfw_ContextMap_Get(GLFWwindow* window)           { for(auto i=0; i < g_ContextMap.Size; i++) { ImGui_ImplGlfw_WindowToContext entry = g_ContextMap[i]; if (entry.Window == window) return entry.Context; } return null; }
...
ImGui_ImplGlfw_Data* ImGui_ImplGlfw_GetBackendData(GLFWwindow* window)
{
    ...
    return cast(ImGui_ImplGlfw_Data*)igGetIO(ctx).BackendPlatformUserData;
}
...
nothrow:
    ...
    bool ImGui_ImplGlfw_ShouldChainCallback(ImGui_ImplGlfw_Data* bd, GLFWwindow* window) {
        try{
    ...
    void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
    {
        try{
            ...
            if (bd.PrevUserCallbackMousebutton !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
    ...
    void ImGui_ImplGlfw_ScrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackScroll !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
            ...
            ImGuiIO_AddMouseWheelEvent(io, cast(float)xoffset, cast(float)yoffset);
...
//#if GLFW_HAS_GETKEYNAME && !defined(EMSCRIPTEN_USE_EMBEDDED_GLFW3)
            ...
            immutable(char)* key_name = glfwGetKeyName(key, scancode);
...
//#endif
            if (key_name && key_name[0] != 0 && key_name[1] == 0)
            {
                ...
                throwIf(char_names.length != char_keys.length);

                if (key_name[0] >= '0' && key_name[0] <= '9')               { key = GLFW_KEY_0 + (key_name[0] - '0'); }
                else if (key_name[0] >= 'A' && key_name[0] <= 'Z')          { key = GLFW_KEY_A + (key_name[0] - 'A'); }
                else if (key_name[0] >= 'a' && key_name[0] <= 'z')          { key = GLFW_KEY_A + (key_name[0] - 'a'); }
                else if (const char* p = strchr(char_names.ptr, key_name[0]))   { key = char_keys[p - char_names.ptr]; }
            }
...
//#endif
    ...
    void ImGui_ImplGlfw_KeyCallback(GLFWwindow* window, int keycode, int scancode, int action, int mods)
    {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackKey !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
            ...
            if (keycode >= 0 && keycode < bd.KeyOwnerWindows.length)
            ...
            ImGuiIO_SetKeyEventNativeData(io, imgui_key, keycode, scancode, 0); // To support legacy indexing (<1.87 user code)
    ...
    void ImGui_ImplGlfw_WindowFocusCallback(GLFWwindow* window, int focused) {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackWindowFocus !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
    ...
    void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow* window, double x, double y) {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackCursorPos !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
            ...
            ImGuiIO_AddMousePosEvent(io, cast(float)x, cast(float)y);
    ...
    void ImGui_ImplGlfw_CursorEnterCallback(GLFWwindow* window, int entered) {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackCursorEnter !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
    ...
    void ImGui_ImplGlfw_CharCallback(GLFWwindow* window, uint c)
    {
        try{
            ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
            if (bd.PrevUserCallbackChar !is null && ImGui_ImplGlfw_ShouldChainCallback(bd, window))
...
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
    throwIf(bd.InstalledCallbacks != false, "Callbacks already installed!");
    throwIf(bd.Window != window);
...
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData(window);
    throwIf(bd.InstalledCallbacks != true, "Callbacks not installed!");
    throwIf(bd.Window != window);
...
bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    bd.Window = window;
    bd.Time = 0.0;
    bd.IsWayland = false;// ImGui_ImplGlfw_IsWayland();
...
//#ifndef __EMSCRIPTEN__
    has_viewports = true;
static if(GLFW_HAS_GETPLATFORM) {
...
static if(GLFW_VERSION_COMBINED < 3300) {
    platform_io.Platform_SetClipboardTextFn = (ImGuiContext*, char* text) { glfwSetClipboardString(ImGui_ImplGlfw_GetBackendData().Window, text); };
    platform_io.Platform_GetClipboardTextFn = (ImGuiContext*) { return glfwGetClipboardString(ImGui_ImplGlfw_GetBackendData().Window); };
} else {
    platform_io.Platform_SetClipboardTextFn = (ImGuiContext*, immutable(char)* text) { glfwSetClipboardString(null, cast(immutable(char)*)text); };
...
static if(_WIN32) {
    HWND hwnd = cast(HWND)main_viewport.PlatformHandleRaw;
    SetPropA(hwnd, "IMGUI_BACKEND_DATA", bd);
    bd.PrevWndProc = cast(WNDPROC)GetWindowLongPtrW(hwnd, GWLP_WNDPROC);
...
static if(_WIN32) {
    ImGuiViewport* main_viewport = igGetMainViewport();
    SetPropA(cast(HWND)main_viewport.PlatformHandleRaw, "IMGUI_BACKEND_DATA", null);
    SetWindowLongPtrW(cast(HWND)main_viewport.PlatformHandleRaw, GWLP_WNDPROC, cast(LONG_PTR)bd.PrevWndProc);
    bd.PrevWndProc = null;
}
...
// #else
        bool is_window_focused = glfwGetWindowAttrib(window, GLFW_FOCUSED) != 0;
// #endif
        ...
        if (is_window_focused)
        {
            ...
            if (io.WantSetMousePos)
                glfwSetCursorPos(window, cast(double)(mouse_pos_prev.x - viewport.Pos.x), cast(double)(mouse_pos_prev.y - viewport.Pos.y));

            // (Optional) Fallback to provide mouse position when focused (ImGui_ImplGlfw_CursorPosCallback already provides this when hovered or captured)
            if (bd.MouseWindow is null)
            {
                ...
                ImGuiIO_AddMousePosEvent(io, cast(float)mouse_x, cast(float)mouse_y);
            }
...
static if(GLFW_HAS_GAMEPAD_API && !EMSCRIPTEN_USE_EMBEDDED_GLFW3) {
    GLFWgamepadstate gamepad;
    if (!glfwGetGamepadState(GLFW_JOYSTICK_1, &gamepad))
        return;
    void MAP_BUTTON(int KEY_NO, int BUTTON_NO, int _UNUSED)       {
        //io.AddKeyEvent(KEY_NO, gamepad.buttons[BUTTON_NO] != 0);
        ImGuiIO_AddKeyEvent(io, cast(ImGuiKey)KEY_NO, gamepad.buttons[BUTTON_NO] != 0);
    }
    void MAP_ANALOG(int KEY_NO, int AXIS_NO, int _UNUSED, float V0, float V1) {
        float v = gamepad.axes[AXIS_NO]; v = (v - V0) / (V1 - V0);
        //io.AddKeyAnalogEvent(KEY_NO, v > 0.10f, Saturate(v));
        ImGuiIO_AddKeyAnalogEvent(io, cast(ImGuiKey)KEY_NO, v > 0.10f, Saturate(v));
    }
} else {
...
void ImGui_ImplGlfw_GetWindowSizeAndFramebufferScale(GLFWwindow* window, ImVec2* out_size, ImVec2* out_framebuffer_scale)
{
    int w, h;
...
public void ImGui_ImplGlfw_NewFrame()
{
    ImGuiIO* io = igGetIO();
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    throwIf(bd is null, "Context or backend not initialized! Did you call ImGui_ImplGlfw_InitForXXX()?");
...
//             viewport->PlatformHandleRaw = (void*)glfwGetCocoaWindow(vd->Window);
    ...
    static if(_WIN32) {
            ...
            vd.PrevWndProc = cast(WNDPROC)GetWindowLongPtrW(hwnd, GWLP_WNDPROC);
            SetWindowLongPtr(hwnd, GWLP_WNDPROC, cast(LONG_PTR)&ImGui_ImplGlfw_WndProc);
    ...
    // See NOTE(1)
    void ImGui_ImplGlfw_GetWindowPos(ImGuiViewport* viewport, ImVec2* outPos)
    {
        ImGui_ImplGlfw_ViewportData* vd = cast(ImGui_ImplGlfw_ViewportData*)viewport.PlatformUserData;
        int x = 0, y = 0;
        glfwGetWindowPos(vd.Window, &x, &y);
        *outPos = ImVec2(cast(float)x, cast(float)y);
    }
    ...
    // See NOTE(1)
    void ImGui_ImplGlfw_GetWindowSize(ImGuiViewport* viewport, ImVec2* outSize)
    {
        ImGui_ImplGlfw_ViewportData* vd = cast(ImGui_ImplGlfw_ViewportData*)viewport.PlatformUserData;
        int w = 0, h = 0;
        glfwGetWindowSize(vd.Window, &w, &h);
        *outSize = ImVec2(cast(float)w, cast(float)h);
    }
    ...
    ImVec2 ImGui_ImplGlfw_GetWindowFramebufferScale(ImGuiViewport* viewport)
    {
        try{
            ImGui_ImplGlfw_ViewportData* vd = cast(ImGui_ImplGlfw_ViewportData*)viewport.PlatformUserData;
...
void ImGui_ImplGlfw_InitMultiViewportSupport()
{
    ...
    // See NOTE(1)
    // GetWindowPos and GetWindowSize return ImVec2 which is not supported by C. We call these
    // helper functions in the cimgui code to set some wrappers
    ImGuiPlatformIO_Set_Platform_GetWindowPos(platform_io, &ImGui_ImplGlfw_GetWindowPos);
    ImGuiPlatformIO_Set_Platform_GetWindowSize(platform_io, &ImGui_ImplGlfw_GetWindowSize);
...
static if(_WIN32) {
    ...
    extern (Windows) LRESULT ImGui_ImplGlfw_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ImGui_ImplGlfw_Data* bd = cast(ImGui_ImplGlfw_Data*)GetPropA(hWnd, "IMGUI_BACKEND_DATA");
        ImGuiIO* io = igGetIO(bd.Context);
        ...
        case WM_XBUTTONDOWN: case WM_XBUTTONDBLCLK: case WM_XBUTTONUP:
            //igGetIO().AddMouseSourceEvent(GetMouseSourceFromMessageExtraInfo());
            ImGuiIO_AddMouseSourceEvent(io, GetMouseSourceFromMessageExtraInfo());
            break;
